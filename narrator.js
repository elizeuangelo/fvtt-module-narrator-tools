...
class NarratorTools {
    ...
    const chatData = {
      content:
        (`<span class="narrator-span${
          type == "narrate" ? " narration" : " description"
        } ${csspatches}">${message.replace(/\\n|<br>/g, "\n")}</span>`),
      type: this.msgtype,
      speaker: {
  ...
  getMessageDuration(length) {
    return Math.clamped(2000, length * 80, 20000) + 3500;
  }
    ...
    if (html.find(".narrator-span").length) {
      ...
      if (html.find(".narration").length) {
        ...
        if (message.data.timestamp + 2000 > timestamp) {
          ...
          this.narratorOpenTimeout = setTimeout(
            this.narratorOpen.bind(this, content, duration),
            500,
          );
          this.narratorCloseTimeout = setTimeout(
            this.narratorClose.bind(this),
            duration,
          );
        }
      } else {
  ...
  narratorOpen(content, duration) {
    this.narratorContent[0].style.fontFamily = `${
      game.settings.get("narrator-tools", "WebFont")
    }`;
    this.narratorContent[0].style.fontSize = `${
      game.settings.get("narrator-tools", "FontSize")
    }`;
  ...
  setScenery(state) {
    if (game.user.isGM) {
      ...
      this.updateScenery();
      game.socket.emit(
        "module.narrator-tools",
        { "command": "scenery", "value": this.scenery },
      );
    }
  }
    ...
    let commands = {
      ...
      update: function () {
        if (game.user.isGM) {
          game.socket.emit(
            "module.narrator-tools",
            { "command": "scenery", "value": Narrator.scenery },
          );
        }
      },
    };
...
Hooks.on("setup", () => {
  ...
  Hooks.on(
    ...
    Narrator._getSceneControlButtons.bind(Narrator),
  );
  ...
  Hooks.on("pauseGame", (pause) => {
    if (game.user.isGM && game.settings.get("narrator-tools", "Pause")) {
      ...
      const btn = $(".control-tool[title=Scenery]")[0].classList.contains(
        "active",
      );
      if (btn != pause) $(".control-tool[title=Scenery]")[0].click();
    }
  });
  // custom languages
...
Hooks.on("ready", () => {
  $(document.getElementById("chat-log")).on(
    "click",
    ".message.narrator-chat",
    Narrator._onNarratorChatClick.bind(Narrator),
  );
    ...
    "items": [
      {
        ...
        "name": "Describe",
        action: () =>
          Narrator.createSpecialChatMessage(
            "describe",
            NarratorTools.getSelectionHtml(),
          ),
      },
      {
        "icon": "comment-dots",
